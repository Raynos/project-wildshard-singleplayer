...
| S3 | **Shard modules (ENGINE-FIT E5).** `src/chunks/<shard>/index.ts` exports config, build, fauna, kit, audio, quest and hooks, loaded by dynamic import. Goal: 0 shard branches outside `src/chunks/`. It also splits the 931 KB gz `main.js` per shard. | L |
| S4 | **Delete the losing variants.** Lighting standard, HDRI sky, cinematic post, look v1, Kuwahara, matte off, and the new `?boat=v1`. WebGPU is either deleted or parked on a branch. The ~123 URL flags go into one typed dev-only registry of about 25. | S–M |

...
| A6 | **Enemy ecology**: respawn on a timer / at night, camps per POI, the guardian boss (the Drowned Captain: a 3-phase fight in the shrine pool) | M | **done** — the Drowned Captain (3-phase boss in the shrine pool, boss bar; `src/entities/species/captain.ts`, mesh remodel queued with the model-agent) + the golden-hour reward view (planet framed in the ring, DayNight eased to golden hour) `fdc2db8`; respawn out of sight in `src/game/quest/Ecology.ts` (vitest timers) `2b8ec4b`; the guarded iron sword (B4 / D6) `5e37e16`; dive treasure re-sunk after W1b `cd230b2` (live cd230b2-mudxevne); progress/180, 185-adv-a6-* |

...
| 0.6 | Viewmodel: sword length / FOV framing to the mockup (short, low-right), one label per enemy, the minimap at the pier spawn | framing **done** `20856c1` (live 20856c1-mudnxer9: low-right, tip below the crosshair; progress/168 → 169; phone calls pier 62 / beach 67 / wreck 113 / shrine 83); one label per enemy + the minimap at the pier: feel-agent, in flight; sword mesh → model-agent |
...
| L4 | **Cloud shadows**: scrolling noise multiplied into the direct light (1 fetch) | S | **done** `5c3ef8e` (live 5c3ef8e-mudo31dq) — drifting 34 m noise patches dim the lit band to 40 %, no texture, no draws; progress/167-look-l4-* |

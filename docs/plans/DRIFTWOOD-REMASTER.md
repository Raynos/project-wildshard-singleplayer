...
| M2 | Every POI to its mockup (`art/driftwood-*` fp + poi rounds): pier, boat, hut (**with an interior**), lookout, bridge, wreck (**with a hold you can enter**), shrine, cove (**open cave mouth**), palms (fluffy sphere-normal fronds, 3-layer wind), boulders, props | **in progress** — WRECK + enterable hold **done** `0b918f5` (progress/168-model-m2-*); SEA CAVE **done** `29f6541` (progress/175); RING SHRINE **done** `4c63d43` (live 4c63d43-mudrvvl6: terraced monolith, the ring frames the planet from the stair head, gull statues, pulsing glyphs (setDusk default 0.35), spring pool + causeway, jungle; anchors altar / pool / stairFoot / ring; phone shrine pose 70 calls / 0.42 M; progress/178); next: hut + lookout → pier / boat |

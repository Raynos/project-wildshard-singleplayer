...
- **B3**, the thralls + the ranger (3 each) → **the ranger = A, the old warden** (grey beard, campaign hat, brass badge, long coat, lantern). Jake said "A or B, whichever we can implement in game". A is chosen because the hat hides the hair and the beard covers the mouth: those are the two things generated humans get wrong, and the ranger only has barks, so no lip sync is needed. The long coat is a near-rigid mass that skins cleanly. B's braid hangs free and needs secondary motion, and her face is more exposed (more uncanny risk). Thralls: open;

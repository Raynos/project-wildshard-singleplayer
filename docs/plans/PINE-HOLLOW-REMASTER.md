...
| PH-C7 | **Night & weather play** | Thralls roam the old-growth at night and flee the dawn. Rain makes animals shelter under the big firs. Dawn fog hides the Ghost Stag. **Built (night)** `8dee497` (`nightThralls.ts`): after dark (night > 0.55) near the old-growth up to 3 (phone) / 4 thralls — the M2 coat, or the King's moss-tinted stand-in — are called up out of sight (≥ 45 m, never in the clearing) and left to the herd AI; when the night lifts they turn, run for the deep trees and are gone in a fog burst; let go past 230 m. Thrall kills count for the contracts and "Weed Control". **Left:** rain sheltering and the dawn fog hiding the Ghost Stag wait on PH-L10's weather. |

...
| # | Row | Gate |
|---|---|---|
| PH-A1 | **Music** (PH-U12): **keep theme 1** (pine calm / tension, 3 styles) + **new slots**: calm-night, the Antler King (3 phase stems), the dawn reward sting; ~6 takes per slot, `rank_v3.py`, demucs stems, bar-exact loops, −18 LUFS, ≤ 5 MB per style | a listening page for Jake (E57 style) — **built, awaiting Jake's veto** `339a484` (code) · `e2beca4` piano · `781b555` orchestral · `82d9f65` folk. 6 MiniMax Music 3 takes per slot. The picks ship to `public/assets/music/pine-hollow-<style>/`: piano 2.66 MB, orchestral 2.78 MB, folk 3.37 MB. The loading bar does not fetch them; Pine Hollow prefetches the selected style once you are in. The page is `art/music/round-3-pine-hollow/index.html` (it has theme 1, calm night, the King's phases I / II / III, the dawn sting and one runner-up per slot). You can trigger the slots with `music.setPineScene('night' / 'boss')`, `music.setBossPhase(1–3)`, `music.sting('dawn')`, or `?music=pine-night` / `pine-boss[-2/-3]` / `pine-dawn`. **Not yet driven by the game.** The day clock and the boss lane call these hooks. |
| PH-A2 | **Zoned ambience**, `ForestAmbience` (the `IslandAmbience` pattern): Hollow, pond, creek / waterfall + the mill wheel, ridge wind, old-growth hush, cave, night (owls, crickets, the thralls' far calls), rain on canopy vs open, dawn birds | zones cross-fade — **the live zones are wired** `339a484` + `e95a942`. `src/audio/ForestAmbience.ts` plays the Hollow, the pond (panned toward the water) and the cabin interiors (the fire, with the outdoor beds muffled through `audio.shadeAmbient`), and crossfades them at 10 Hz. It is main.ts's ambience on Pine Hollow. **Ready as data** (`amb.addSpot`, `amb.night / dawn / rain / thralls`): creek, waterfall, mill, ridge, old-growth, cave, night (plus the thralls' far calls), rain on the canopy vs in the open, and dawn. 14 beds in `public/assets/sfx/pine-hollow/` (3.32 MB with the one-shots and barks). |
| PH-A3 | **SFX set**: crossbow, lever-action (cycle, shot, the ridge echo), longbow; every animal voice (elk bugle, bear roar, boar squeal) + the thralls + the King (antler-bells, root stomps); footsteps on needle / mud / wood / rock / wet; doors, lanterns, the zipline, the waterfall, the mill | `sfx-best.json` per sound — **done** `73dd222` + `e95a942`. MOSS v2 and SA3 Medium each made 3 takes per sound; CLAP picks the better one (`scripts/music/gen/sfx-ph-jobs.json`, rows in `sfx-best.json`). **Wired now** (merged into `best/`, +0.25 MB): the crossbow loose, draw and hits on wood, ground and flesh; the elk, the bear roar / growl and the boar squeal; footsteps on needles, planks, mud, rock and wet (`ForestAmbience.stepSurface`). No new take won for deer_call and boar_grunt, so the round-2 takes stay. **Ready** in `sfx/pine-hollow/` (`ambience.sfx.shot(…)`): the lever-action (cycle, shot, ridge echo), the longbow draw, the deer snort, the thralls, the King (bells, stomp, roar), the doors, the lanterns and the zipline. Waterfall and mill are beds. boltImpact-rock and longbowLoose ranked outside the top 5, so the synth keeps them. |
| PH-A4 | **NPC barks** (PH-U23): the ranger, the miller and the trader, 6–10 short generated barks each | versus-take page — **done** `e95a942`. 21 barks ship (ranger 6, miller 7, trader 8). CLAP ranks them as one group because it cannot tell one voice from another. They play through `ambience.sfx.bark(npc, at)`, which never repeats the last one, and are on the listening page next to both engines' takes. The NPCs do not exist yet. |
| PH-A5 | **Reverb IRs**: cabin, cave, old-growth, the bowl | on zone entry — **done** `339a484`. The IRs are generated like Driftwood's: `gen.ts` rooms cabin 0.5 s, den 1.8 s, oldgrowth 1.4 s, bowl 0.9 s (0 bytes). **The cabin reverb is wired** on interior entry, and a light bowl slap is live across the Hollow. The den and the old-growth follow their spots once the layout adds them. |

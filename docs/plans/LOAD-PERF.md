...
**State:** `in progress` 2026-09-22 — finish line (the user, 2026-09-22: "default shard only"): every row of `bench.budget.json` passes on `pnpm bench:ci -- --query=tier=phone --viewport=390x844` (4× CPU, wifi + 4G, cold + warm) for **Driftwood Isle, the default shard**, **and** one iPhone reading of a warm launch ≤ 3 s; Pine Hollow (experimental) is best-effort. The user also kept the loading screen up until the world is compiled (no finishing behind the title menu, §P4.5). At `fb4fc6d` Driftwood is **6 / 8**: title 0.53 s, 3.07 MB, 17 requests, warm transfer 0, cold 4G 5.55 s, SW pass; fails warm 4G 3.13 s and longest task 233 ms. The two tasks over 100 ms (CDP profile, 4× warm): `Rifle.buildViewmodel`'s procedural textures 228 ms (E1 weapons bake, queued) and the baked-sky decode's getImageData 122 ms (`BakedSky.ts`, E1 cold-bytes, asked). Pine Hollow at `847ad3a`: title, 11.74 MB, 17 requests, warm transfer, SW pass; cold 4G 14.2 s, warm 4G 4.18 s, longest task 360 ms.

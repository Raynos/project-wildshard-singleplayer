...
**State:** `blocked` 2026-09-22 — on the user's iPhone reading only. Finish line (the user, 2026-09-22: "default shard only"): every row of `bench.budget.json` on `pnpm bench:ci -- --query=tier=phone --viewport=390x844` (4× CPU, wifi + 4G, cold + warm) for **Driftwood Isle, the default shard** — **8 / 8 PASS** on HEAD + the entry split (quiet host, load ≈ 3): title 0.46 s · first launch 2.38 MB · warm transfer 0 · 17 requests · cold 4G 4.29 s · warm 4G 1.72 s · longest task 61 ms · SW controls the warm run — **and** one iPhone home-screen reading of a warm launch ≤ 3 s (the user's). Pine Hollow (experimental) is best-effort: 11.7 MB, 17 requests, the pack and every CPU lever apply. The user kept the loading screen up until the world is compiled (§P4.5 not taken).
...
| **Fogless viewmodel depth clearers + hoverboard halo** (the sword / hoverboard / rifle / hands clearers and the hoverboard's additive halo were the only fogged MeshBasic users: a FrontSide + BackSide pair of programs) | `8df5524` | tier=phone Pine Hollow **73 → 71**, Driftwood **76 → 74**; 0 at the first frame; look unchanged (progress/146 → 147) | _pending_ |
| **Viewmodel textures drawn in a worker** (crossbow walnut / steel / leather / cord / bolt + rifle anodised / polymer / steel: code moved unchanged to `viewmodelTextures.ts`, an inline `?worker&inline` worker started at the head of the boot, awaited by the weapon step; main-thread fallback). Not a PNG bake: 23 planes = 4.7 MB lossless, and JPEG would alter the normal / ARM data | `bc376b7` | 4× CPU tier=phone: the weapon step's 443 ms task (Pine Hollow; Rifle.buildViewmodel 228 ms on Driftwood) gone — no weapon-step long task on either shard, prod Driftwood included; 0 bytes, 0 requests; every plane byte-identical to a main-thread draw | _pending_ |
...
| **P5 · prop textures q99 → q85** (`scripts/tex-tiers.mjs` §2: the glTF props' 1k JPEGs and the JPEGs inside `<id>_lod.glb` re-encoded in place, normal maps 4:4:4; both tiers, same 1024 px) | `1c4d87b` | −12.7 MB on disk; stump LOD 3.07 → 1.12 MB, trunk LOD 2.59 → 0.88 MB | part of the P5 cold total below |
| **P5 · sky HDR → gain-mapped pair** (`bake-sky.mjs` `encodeSky`: `<hdri>_2k.sky.jpg` colour ÷ gain, sRGB q95 4:4:4 + `_2k.gain.png` 8-bit log2 gain, 0–16 stops; `src/world/BakedSky.ts` rebuilds HDRLoader's half-float DataTexture in a Worker; PMREM stays at runtime, split into 3 tasks) | `1374a41` `8b57364` `8362d8a` | Pine Hollow sky 4.17 → 0.29 MB, Driftwood 5.45 → 0.41 MB; mean per-pixel error vs the .hdr 1.5 % (p99 3.8 %); sky step at 4× CPU: 128 ms decode task → 0 (Worker), PMREM 130 ms task → ≤ 64 ms pieces | same-camera screenshots match |
| **P5 · phone copies** (`tierUrl()` in `src/boot/bytes.ts`: `.png/.jpg → .phone.webp`, `.glb → .phone.glb` when the build has one; fetchImage, three's `DefaultLoadingManager` URL modifier and the boot manifest all go through it. `tex-tiers.mjs` §3–4: WebP q75 `-sharp_yuv` from the best source (≈ 22 % under the q82 JPEG *and* lower error: 0.024 vs 0.032), PNG alpha lossless `-exact`, ≤ 1024 px, AO/rough/metal at 512 px, LOD GLBs with EXT_texture_webp) | `1374a41` `81fdb22` | desktop untouched; phone copies 22.1 → 8.1 MB on disk | Pine Hollow tier=phone screenshots (golden-hour sky, sunlit cabin, log wall + woodpile at 3 m) show no difference — progress/149 → 150 |
| **P5 · SW precaches code, not images** (install skips the hashed hero stills; cacheFirst stores them when the menu shows one; BUNDLE still the prune list, static-cache migration untouched) | `50b9326` | −1.9 MB from a `skipintro` first launch | — |
| **P5 · manifest per shard + Driftwood sky textures baked** (`chunkFiles` declares only what the shard reads: low-poly → baked terrain only, treeless → no tree textures, ocean → no cabins/props; clouds + gas giant via `bake-textures --chunk driftwood-isle`, inside the boot pack) | `1da15cd` | Driftwood DOWNLOAD total ~20 MB declared → the ~2 MB it reads; sky clouds/planet no longer drawn at launch (399 ms task at 4×) | — |
| **P5 · cold first launch, tier=phone, measured** (`bench-load.mjs --query=tier=phone&skipintro=1&chunk=<slug>`, live, no throttle, Playwright encoded body bytes incl. SW fetches) | `8894c70` → `1da15cd` | **Pine Hollow 37.47 → 10.01 MB** (before: jpg 20.2 · glb 6.6 · png 5.2 · hdr 4.0 · bin 1.0 · js 0.6; after: boot pack 8.97 · js 0.56 · icons 0.39 · fonts 0.07); **Driftwood 1.95 MB**, 11 requests | E4 reads 11.74 MB at 390×844 without `skipintro`: the title screen fetches all 9 hero stills (every shard's portrait + landscape + thumb, 2.06 MB) — open |
| **P5 · not taken, and why** | — | **KTX2/Basis**: at equal bytes ETC1S has 2.4× WebP's error (forest_ground_04 1k: ETC1S q255 266 KB err 0.058 vs WebP 266 KB 0.024; ETC1S q128 176 KB 0.098), UASTC is ~4× the bytes, plus ~0.5 MB of transcoder — it buys GPU memory, not a smaller download. **Baked PMREM**: the cube-UV target would be *extra* bytes on top of the background equirect; runtime PMREM is 3 short tasks now. **512² terrain layers on the phone**: A/B at a ground camera, the ground visibly softens — kept 1024. | — |
| **three.js evaluates in its own task** (`src/boot/entry.ts` is the page entry: `import('three')`, a macrotask, then `import('../main')` — the whole static graph used to evaluate as one task before the first boot step; `cssCodeSplit: false` keeps it one stylesheet) | E4 | Driftwood tier=phone 390×844 4× CPU, same-host A/B: longest task wifi/cold 120–132 → 55–61 ms, warm 143–168 → 76–122 ms; requests 15 → 17 (three.module chunk + main chunk) | _pending_ |
| Title menu: only the selected card's hero art (neighbours on a swipe, the other orientation on a flip) | `62f66b8` | Driftwood cold 3.07 → 2.35 MB, 17 → 15 requests (−1.7 MB on every shard's first launch) | _pending_ |
| Cabin geometry merge (§P1) | — | — | — |

...
| 6 | SFX / ambience feasibility with MiniMax: ambient beds, pickup / shrine hum, a test take for each one-shot family; ship what convinces, report what doesn't | done `e8b0925` — the user moved SFX off MiniMax (music only) to two SFX models, both in the game: **Stable Audio 3 Small-SFX** in `public/assets/sfx/sa3/` (25 of 51 families) and **Stable Audio 3 Medium** in `sfx/sa3-medium/` (45 of 51); 3 takes per family ranked by CLAP, a family ships only in CLAP's top 5, else it is listed in `synth_keeps`. TangoFlux's licence is non-commercial (research only), so Medium replaced it: the game's `tangoflux` set must be renamed `sa3-medium`. Snippets side by side: `art/sfx/round-1-two-models/` |

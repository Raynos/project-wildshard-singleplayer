...
| 6 | SFX / ambience feasibility with MiniMax: ambient beds, pickup / shrine hum, a test take for each one-shot family; ship what convinces, report what doesn't | done (round 2, E33) — **three local SFX models whose licences allow commercial use, all in the game** (Settings → Sound effects: MOSS · SA3 Medium · EzAudio · Synth): **MOSS-SoundEffect v2.0** (Apache-2.0) `public/assets/sfx/moss/`, the default — 46 of 51 families, 26 ranked first by CLAP, mean rank 2.7; **Stable Audio 3 Medium** (Stability Community) `sfx/sa3-medium/` — 45, 24, 3.3; **EzAudio-XL** (MIT) `sfx/ezaudio/` — 17, 9, 17.0. 3 takes per family, CLAP-ranked, a family ships from a set only in CLAP's top 5, else `synth_keeps`. Small-SFX (round 1) dropped. MiniMax Music 3 tried on 10 families (1 take each): best rank 5, wins none, not shipped. Rejected on licence: TangoFlux, ThinkSound (commercial use not permitted), AudioX / Tango 2 / AudioLDM 2 (CC-BY-NC), Foley-Omni (ships MMAudio parts, CC-BY-NC), HunyuanVideo-Foley (outputs may not be used in the EU / UK / South Korea). Comparison: `art/sfx/round-2-top3/` |
...
| 10 | Re-cut the 15 s / 30 s trailers with the new score; deploy; the user listens on the phone | in progress `e507b24` / `e8b0925` — `public/trailer-{15,30}.mp4` keep their picture; the audio is the piano title, placed so its biggest downbeat swell lands on the first sword swing (12.8 s / 5.3 s), low-passed under water (`scripts/trailer/score_minimax.py`). Open: the user listens on the phone |

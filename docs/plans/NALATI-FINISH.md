...
**State:** `in progress` 2026-09-24 — Nalati is on main (b3d4e00, live). Done: U1–U2, P2 (score), P3 (defaults). P1 = the N20 lock-in is done (shadows OFF, AO OFF, model shading ON, procedural yurts, image-to-3D camp people). B6 = N23 hide the slab edge is built (Look Lab ▸ Edge, default OFF) and waits on the user's pick (board: progress/nalati-merge/n23/board.jpg). Next session: B5 (camp people's faces), B1–B3, B4; the user: P4 (N11, Low Power Mode off), U3 (delete the branch), his bug list.

...
| # | Row | Why it moved |
|---|---|---|
| F-P5 | ~~**WebGPU**: Pine Hollow on the TSL path~~ | **dropped 2026-09-25 (E184)**: Jake had every WebGPU path deleted — `src/gpu/` is gone and the Renderer picker with it |
...
| F-P6 | **Fights under 30 on the iPhone**: in the Ironhide fight Jake's panel read gpu~ 35–48 ms (0 walking), update 5–6, render 7–8 — the GPU / iOS compositor, not the AI — but with the iOS screen recorder on. Shipped (4ff49d2): the elite skull's infinite CSS pulse, the per-frame DOM writes / layout reads, the bone-matrix skip. Unconfirmed on the phone. If Jake says fights still lag: automate the check (REC 30 S cycles the A/B suspects itself, 3 s each between baselines, and the COPY text names the one that drops gpu~) — **Jake will not toggle switches by hand** | E142 aggro-perf lane |
...
| F-G5 | Foliage depth pre-pass (alpha-tested depth only, then colour at EQUAL) | cards cost 0.12–0.92 ms (old-growth 9 alpha layers kept, 20 rasterised per px); the 30-fps-at-2× lane: their lighting is only 0.13–0.27 of 0.35–0.65 ms in the forest | ~0 net on the M5 (the pre-pass re-rasterises every layer with the alpha test); front-to-back sort shipped instead (−0.08…−0.12) |

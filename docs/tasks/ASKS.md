...
| V3 | **Elk species** — "a bigger beast, behaves like the deer but way harder to hunt — faster, moves away faster, way higher difficulty" | **done** | `cd6462a` (+ `96f0272`) — `src/entities/species/elk.ts` (registry, one file): 1.5 m shoulder, heavy neck + dark shaggy mane ('crest' bib), pale tan barrel / dark legs + belly / cream rump, 6×6 back-swept antlers. **Cow** 50 % 160 hp · **Bull** 38 % 200 hp · **Royal bull** uncommon 8 % ×1.25 260 hp antlers ×1.2 · **Pale elk** rare 3 % cream coat · **Imperial bull** legendary 1 % ×1.4 340 hp antlers ×1.4 gold rim/glow speed ×1.15. Body hits ×0.8 on every variant. ELK_TUNING (not the deer baseline): sight 45 m / 70° (24 m grazing), hearing 4/10/22/40 m, noticeRate 0.6 (2× deer), freeze 0.6–1.2 s, panic 14 m, run 11 m/s to 90–140 m, wary 60 s, herd alert 40 m. Measured: a walker makes a grazing group bolt at ~22 m and they stop 100–136 m away; deer beside them never looked up before 14 m. Bulls bugle every 40–120 s (`elk_bugle` in Audio.ts; registry `sounds.callVariants/callEvery`). Shots: progress/elk/elk-contact.png |
| V4 | **Fauna redistribution: many small groups across the whole chunk** — "last time the agent just made each sparse cluster bigger — nine boars in the middle of nowhere; I don't want to search endlessly in an empty forest" | **done** | `dfc9b1f` — `src/chunks/fauna-layout.ts`: an 8×8 grid of cells 60 m apart (25 m inside the edge), pond / cabin pads / south gate skipped, ±15 m seeded jitter, one small group per cell by weighted roll (deer 3–4, boar 2–3 under canopy, elk 2–4 near the trail clearings, or empty 10 % — never two empties side by side). AnimalManager spaces anchored plans by their own 20 m ring instead of 60 m. Pine Hollow now: **54 groups — 87 deer · 51 boar · 24 elk · 3 bears = 165**; every 100 m bucket of the 5×5 grid holds 2–12 animals (max 7 %, none empty). Headless desktop fps 35–37 with 165 vs 57 animals active (not the bottleneck; fur shells capped at the 4 nearest). progress/090-fauna-south-trail.png |
...
| G2 | (update) user: "those mockups look great — **implement that**" | **in flight** | src/ui/Menu.ts + gmenu.css + src/game/{achievements,Progress,Inventory}.ts; HUD.ts / main.ts wiring |

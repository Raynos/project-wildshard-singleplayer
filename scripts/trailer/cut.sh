#!/bin/bash
# Editor's cuts from the same frame folders. usage: cut.sh <frames dir> <edl file> <out.mp4> [bpm] [score.wav]
# EDL lines:  card <id> <secs>   |   vision <file> <start> <secs>   |   shot <name> <startFrame> <nFrames>
# With a 5th arg the score (scripts/music/render.mjs → score-30.wav / score-15.wav: the real theme rendered to the cut)
# is the audio track, faded out over the last second, instead of the lavfi synth bed.
set -e
cd "$(dirname "$0")"
F=$1; EDL=$2; OUT=$3; BPM=${4:-96}; SCORE=$5
...
while read -r kind a b c; do
  ...
  case "$kind" in
    ...
    # the vision sources are 8 fps: minterpolate comes up ~7 frames short of `-t`, so pad with the last frame and cut to the exact count (the score is timed to the EDL)
    vision) ffmpeg -nostdin -v error -y -ss "$b" -t "$c" -i "$SRC/$a" -vf "scale=1600:900:force_original_aspect_ratio=increase,crop=1600:900,minterpolate=fps=$FPS:mi_mode=blend,tpad=stop_mode=clone:stop_duration=1,fade=t=in:st=0:d=0.3,fade=t=out:st=$(python3 -c "print(max(0,$c-0.3))"):d=0.3,format=yuv420p" -r $FPS -frames:v "$(python3 -c "print(round($c*$FPS))")" -c:v libx264 -crf 18 -pix_fmt yuv420p "$T/$n.mp4" ;;
...
if [ -n "$SCORE" ]; then
  ffmpeg -v error -y -i "$T/video.mp4" -i "$SCORE" -af "afade=t=out:st=$(python3 -c "print(max(0,$DUR-1.0))"):d=1.0" -c:v copy -c:a aac -b:a 160k -shortest -movflags +faststart "$OUT"
  echo "wrote $OUT (${DUR}s, score $SCORE)"; rm -r "$T"; exit 0
fi

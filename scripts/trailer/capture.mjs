...
const shots = [
  ...
  { name: 'boars', url: `${D}&x=45&z=-140&yaw=3.1416&pitch=-0.02`, secs: 6,
    ...
    tick: (i) => `(() => { const b = window.__boar; if (b) { const p = window.__world.player; const want = Math.atan2(-(b.position.x - p.position.x), -(b.position.z - p.position.z)); p.yaw += (want - p.yaw) * 0.05; } })();` },
...
for (const s of shots) {
  ...
  await page.waitForFunction(() => !!window.__world && !!window.__hf, null, { timeout: 180000 });

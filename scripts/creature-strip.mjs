#!/usr/bin/env node
// creature-strip.mjs — every gait of a creature as a frame strip, rendered IN the live shard, so a rig is judged in motion
// (the generated hulls skinned to the procedural skeletons). Shard-agnostic copy of Nalati's
// scripts/nalati-creature-strip.mjs: the chunk picks the spot and the query (PINE-HOLLOW-REMASTER PH-M1).
//
// For each subject it makes one Animal (the game's own factory + Animal.ts, no AI), stands it on the chunk's strip spot
// and poses it frame by frame: 4 views, idle / graze, walk / trot / gallop at 4–5 phases each, the gallop 3/4 from the
// front, the attack at 4 points and the death at 3, each shot side-on. Output: one sheet per subject,
// <out>/<kind>-<variant>-<tag>.jpg (rows = gaits, columns = phases). `--coats=1`: one row per subject (side idle, side
// walk, 3/4 gallop), every subject on one sheet: <out>/coats-<tag>.jpg. `--weights=1`: the skin weights as colours.
//
//   node scripts/creature-strip.mjs --chunk=pine-hollow --only=deer:hind,boar:boar --tag=glb
//   node scripts/creature-strip.mjs --chunk=pine-hollow --only=deer:hind --tag=proc --creatures=proc
//   --url=http://127.0.0.1:5176  --tier=phone  --rows=walk,gallop  --q=tod=night
...
/** per chunk: an open, level spot to stand the subject on, the query that freezes its clock / weather, the out folder */
const CHUNKS = {
  'pine-hollow': { spot: { x: 6, z: -178 }, q: 'tod=day&clock=1e6', out: 'progress/pine-hollow-creatures', subjects: 'deer:hind,boar:boar' },
  'nalati-grasslands': { spot: { x: 41, z: -150 }, q: 'weather=clear&clock=0', out: 'progress/nalati-look/creatures', subjects: 'wolf:grey,horse:dun' },
};
const CFG = CHUNKS[CHUNK];
if (!CFG) throw new Error(`creature-strip: no table for chunk '${CHUNK}'`);
const OUT = resolvePath(ROOT, flag('out', CFG.out));
...
const EXTRA = flag('q', '');
const only = flag('only', CFG.subjects).split(',').filter(Boolean);
const SPOT = { x: Number(flag('sx', String(CFG.spot.x))), z: Number(flag('sz', String(CFG.spot.z))) };
const CELLS = flag('cells', '');
if (CELLS) mkdirSync(CELLS, { recursive: true });
const CW = Number(flag('cw', '420')), CH = Number(flag('ch', '300')); // one cell
...
const ROWS = COATS ? COAT_ROWS : ROWSEL ? ROWS_ALL.filter((r) => ROWSEL.split(',').includes(r.label)) : ROWS_ALL;

async function compose(browserRef, rows, sheetTitle, maxBytes) {
  const comp = await (await browserRef.newContext()).newPage();
  const b64 = await comp.evaluate(async ({ rows: rs, cw, ch, title, max }) => {
    ...
    const G = 4, TOP = 28, LAB = 20;
    const c = document.createElement('canvas'); c.width = maxC * (cw + G) + G; c.height = TOP + rs.length * (ch + LAB + G);
    ...
    for (let r = 0; r < rs.length; r++) {
      ...
      for (let i = 0; i < row.cells.length; i++) {
        ...
        const tag = f.attack !== undefined ? `attack ${f.attack}` : f.death !== undefined ? `death ${f.death}` : `${f.gait} ${f.phase}${f.view ? ` ${f.view}` : ''}`;
        x.font = '600 13px ui-monospace, monospace'; x.fillStyle = '#9fe6ff'; x.fillText(`${row.label} · ${tag}`, G + i * (cw + G) + 2, y + 15);
      }
    }
    for (let qq = 0.85; qq >= 0.3; qq -= 0.05) { const u = c.toDataURL('image/jpeg', qq); if (u.length * 0.75 < max) return u.split(',')[1]; }
    return c.toDataURL('image/jpeg', 0.25).split(',')[1];
  }, { rows, cw: CW, ch: CH, title: sheetTitle, max: maxBytes });
  await comp.close();
  return b64;
}
...
try {
  ...
  await page.addStyleTag({ content: '#hud,#hud *,.ws-touch,[class*="elite"],[class*="banner"],[class*="toast"],[class*="prompt"]{display:none!important}' });
  await page.evaluate(async (spot) => {
    ...
    window.__csMake = (kind, variant, weights) => {
      if (st.cur) st.cur.mesh.parent?.remove(st.cur.mesh);
      const f = w.animals.factory;
      let model = f.model(kind, variant);
      if (weights) {
        // weight view: each bone its own colour, blended by the skin weights (the atlas off)
        const pal = (b) => { const n = model.bones[b]?.name ?? ''; const c = new THREE.Color();
          ...
          else if (n.startsWith('tail')) c.setHSL(0.9, 0.9, 0.55);
          else if (n.startsWith('neck')) c.setHSL(0.58, 0.8, 0.45);
          else if (n === 'head' || n.startsWith('ear')) c.setHSL(0.5, 0.9, 0.6);
        ...
        const fur = model.fur.clone(); fur.map = null; fur.normalMap = null;
        model = { ...model, geometry: g, fur, shells: [] };
      }
      ...
      st.cur = a;
      if (model.shells.length > 0) { a.makeShells = () => f.createShells(rig, model); a.setShellLevel(4); }
      ...
      return { hull: model.hull !== undefined, h: b.max.y - b.min.y, len: b.max.z - b.min.z, tris };
    };
    w.game.onUpdate(() => {
    ...
    window.__csFrame = (fr, h, len) => {
      st.frame = fr;
      const d = (Math.max(h, len) * 1.55 + 0.4) * spot.zoom;
      ...
      if (fr.view === 'back') {
        ...
        st.pose = { x: spot.x, y: ty + h * 0.12, z: spot.z - d, tx: spot.x, ty, tz: spot.z }; // the animal's left side
      }
    };
  }, { ...SPOT, zoom: Number(flag('zoom', '1')) });

  const allRows = [];
  for (const subj of only) {
    const [kind, variant] = subj.split(':');
    const info = await page.evaluate(([k, v, wt]) => window.__csMake(k, v, wt), [kind, variant, flag('weights', '0') === '1']);
    console.log(`${subj}: hull ${info.hull} tris ${info.tris} h ${info.h.toFixed(2)} len ${info.len.toFixed(2)}`);
    const rows = [];
    for (const row of ROWS) {
      ...
      for (const fr of row.frames) {
        ...
        // --cells=<dir>: every cell also as its own JPEG (the versus boards take single frames)
        if (CELLS) writeFileSync(resolvePath(CELLS, `${kind}-${variant}-${TAG}-${row.label.replaceAll(/\W+/g, '')}-${cells.length - 1}.jpg`), Buffer.from(cells.at(-1) ?? '', 'base64'));
      }
      rows.push({ label: row.label, cells, frames: row.frames });
    }
    if (COATS) { allRows.push(...rows.map((r) => ({ ...r, label: subj }))); continue; }
    const b64 = await compose(browser, rows, `${subj} · ${TAG} (?creatures=${CREATURES}, ${TIER}) · ${info.tris} tris`, 4.8e5);
  ...
  if (COATS) {
    ...
    writeFileSync(out, Buffer.from(await compose(browser, allRows, `coats · ${TAG} (?creatures=${CREATURES}, ${TIER})`, 4.8e5), 'base64'));
    console.log(`  wrote ${out.slice(ROOT.length + 1)}`);
  }

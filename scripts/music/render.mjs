...
for (const [name, seconds, out, solo, state] of jobs) {
  const t = Date.now();
  const b64 = await page.evaluate(([n, s, so, st]) => window.__music.renderWav(n, s, so !== undefined && so.length > 0 ? so : undefined, st), [name, seconds, solo, state]);

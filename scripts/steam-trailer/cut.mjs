...
//   node scripts/steam-trailer/cut.mjs <outDir> <musicTake.wav> <sfxDir>   → edl.json, titles.json, mix.json
//
...
const [OUT, TAKE, SFX] = process.argv.slice(2);
...
writeFileSync(`${OUT}/edl.json`, JSON.stringify({ length: LEN, clips, titles }, null, 1));
writeFileSync(`${OUT}/titles.json`, JSON.stringify(titles, null, 1));
writeFileSync(`${OUT}/mix.json`, JSON.stringify(mix, null, 1));
console.log(clips.map((c) => `${c.at.toFixed(2).padStart(6)}  ${c.shot.padEnd(11)} in ${c.in}  ${c.dur.toFixed(2)} s`).join('\n'));
console.log(`${ev.length} sound events; end card at ${HIT.toFixed(2)}`);

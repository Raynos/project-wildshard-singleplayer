...
// src/boot/versions.generated.ts: a content hash per file under public/assets/nalati/ (NALATI-MERGE F4). Those files are
// unhashed and get edited in place (a re-baked GLB, a repainted texture), so the network sees them as `?v=<hash>`
// (src/boot/bytes.ts `versionedUrl`): an edit is a new URL to the HTTP cache (vercel.json: immutable) and the service
// worker (cache-first), and reaches a returning player on the next deploy.
function writeVersionsModule() {
  ...
  const walk = (dir: string, pub: string) => {
    ...
    for (const name of readdirSync(dir).sort()) {
      ...
      else if (name !== '.DS_Store') rows.push(`  ${JSON.stringify(`${pub}/${name}`)}: '${createHash('sha256').update(readFileSync(p)).digest('hex').slice(0, 8)}',`);
    }
  };
  walk('public/assets/nalati', '/assets/nalati');
  const src = `// generated by vite.config.ts from public/assets/nalati — do not edit. Regenerated by every vite build / dev.\nexport const ASSET_VERSIONS: Readonly<Record<string, string>> = {\n${rows.join('\n')}\n};\n`;

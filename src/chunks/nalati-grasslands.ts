...
/** the ground the splat reads: the finished field (N23's berm on top), set once built */
...
/** N23: the edge berm's rise at (x, z) (0 before the terrain is built) — the spruce lines and the crest's granite read it */
export function edgeBermAt(x: number, z: number): number {
  return underBerm ? edgeRise(x, z, underBerm.heightAt(x, z)) : 0;
}

/**
 * modelProps — the camp pieces that are generated GLB models (glbPaint.ts): the kazan, chests, woodpile, churns, ground
 * saddles and the eagle. They replaced PaintKit twins (E136 deleted those, the user's pick) and kept their footprints,
 * colliders and return values (the fire's centre), so the smoke, the yard wear and the player's collision are the
 * same. The colliders are pushed at build time; the model goes into a `ModelSink` (one InstancedMesh per model per
 * POI) and appears when its GLB has loaded (a failed load draws nothing and logs once; the collider stays).
 *
 *   const sink = new ModelSink();
 *   addChestModel(sink, ground, x, z, yaw, colliders);
 *   …  void sink.flush(group, sky);
 *
 * The GLBs face +Z; the POI kit faces −z at yaw 0, so every piece turns the model by `yaw + π`.
 */
...
/** the kazan on its tripod over the fire ring (the cauldron GLB); returns the fire's centre for a plume */
...
/** a firewood stack: two birch stacks side by side along the pile */
...
/** the kumis corner: a churn by the yurt and a bigger one 1.1 m east of it */
...
/** a saddle set down on the grass (no collider) */
...
/** the golden eagle standing on `feet` (a perch), facing `yaw` (0 = −z) */

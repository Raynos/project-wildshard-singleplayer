...
export class Cove {
  ...
  build(spec: CoveSpec): this {
    ...
    if (smoothRocks.length > 0) {
      // they keep their own smooth normals: all the loose rocks are one more draw
      const sm = new THREE.Mesh(mergeGeometries(smoothRocks, false), rockMaterial(this.sky));

...
/**
 * The paint GLSL (after NOISE_GLSL: uses h12 / vnoise). Include once in a program that has `uPaint` / `uPaintK` /
 * `uPaintK2` and `uInk0`. All helpers return a MULTIPLIER for the wash (mean 1) except `paintPoster`.
 */
...
vec4 paintFlag(vec2 p, float k) {
  ...
  float Ls = 1.05 + 0.55 * h11(r * 9.1 + 2.0);
  float cx = floor((p.x + off) / Ls);
  vec2 cid = vec2(cx, r);
  vec2 loc = vec2(p.x + off - cx * Ls, p.y - r * rh);

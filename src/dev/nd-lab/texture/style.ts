// P5 texture lab: a copy of the clean room's style.ts; every change is marked `P5 paint`.
...
import { PAINT_GLSL, paintUniforms } from './paint'; // P5 paint
...
const BANDS: readonly Band[] = [
  { y: 212, w: 6, d: 0.012, jiehua: 0xd6dbe2, silk: 0xd8cfbd, sutra: 0x2a3a62 },
  { y: 152, w: 3, d: 0.01, jiehua: 0xc9ced6, silk: 0xcbc0ad, sutra: 0x24345a },
  { y: 101, w: 4, d: 0.07, jiehua: 0xc4c7cc, silk: 0xc6baa7, sutra: 0x223257 },
  { y: 36, w: 4.5, d: 0.08, jiehua: 0xa9b0bb, silk: 0xaba396, sutra: 0x1e2c4e },
  { y: -30, w: 5, d: 0.085, jiehua: 0x8e97a6, silk: 0x8f8a82, sutra: 0x192644 },
...
const LOOKS: Readonly<Record<LookName, LookPreset>> = {
  jiehua: { fog: 0x9aa6ba, sky: [0x5d6d8a, 0x9fabbf], tint: [0.9, 0.93, 1.0], shade: 0xa4abbb },
...
export class Shared {
  readonly u = {
    ...
    uGroundY: { value: 0 },
    // P5 paint: the painted-surface array and its strengths (paint.ts); the glazed tiles' pitch (the texture's 7 × 7)
    ...paintUniforms(),
...
vec4 silkFog(vec3 wp, float scale) {
  ...
  float a0 = 1.0 - exp(-uFogBase * max(L - uFogStart, 0.0) * scale);
...
void main() {
  ...
  // P5 paint: the painted surface (flag bits × 4096: 0 by kind, 1 none, 2 stone, 3 concrete, 4 lacquer, 5 wood, 6 poster)
  ...
  if (kind == 1.0) {
    ...
    // P5 paint: the wall is painted concrete (2 scales, grime as ink); glass and lit rooms keep their colours
    ...
    // P5 paint: the rulings take the glazed-tile texture's pitch so its joints and the ink coincide
    ...
    if (pk > 0.0) col *= paintTiles(q, vec2(cp, rp), pk * uPaintK.w);
    ...
    // P5 paint: painted granite per stone (two layers, a window and a quarter turn each); its cavity deepens puddles
    vec4 pf = paintFlag(p, pk * uPaintK.y);
    col = base * (0.82 + 0.3 * st.y) * mix(0.74 + 0.52 * speck, 1.0, pk) * pInk(pf.rgb, uPaintInk);
    float wAmt = wet * mix(0.55, 1.0, clamp(st.z + (pf.a - 0.5) * 1.6 * uPaintK2.w * pk, 0.0, 1.0));
    col *= 1.0 - 0.55 * wAmt;
    float ndv = clamp(V.y, 0.0, 1.0);
    float fres = 0.04 + 0.96 * pow(clamp(1.0 - ndv, 0.0, 1.0), 5.0);
    // P5 paint: the sky sheen gathers in the wet (dark) dabs of the painted granite, the pale dry dabs lose it
    float dab = mix(1.0, clamp(2.1 - 1.15 * dot(pf.rgb, vec3(0.2126, 0.7152, 0.0722)), 0.15, 1.8), pk * uPaintK2.w);
    // (and a painted wet floor never goes fully matte: the fresnel keeps a floor of sheen seen from above)
    emit += fogCol(vWorld) * mix(fres, 0.14 + 0.86 * fres, pk) * wAmt * mix(0.3, 0.42, pk) * (1.0 - st.x) * dab;
    vec2 rc = floor(p / 1.1);
    ...
    // P5 paint: which paint a panel takes — a painted board (accent) or timber (surf wood); a balustrade-sized stone
    // panel (0.3–1.2 m tall, 1.8–8 × as wide) the carved frieze over its whole face; any other stone box plain stone
    ...
    float frieze = pk * friezeFace * (1.0 - isBoard);
    ...
    // P5 paint: the frieze's own carved frame, lit lips and inked relief replace the ruled inset frame and the
    // procedural ruyi (they fade out under it); boards take weathered lacquer or planks
    if (pk > 0.0 && isBoard > 0.5) {
      col *= surf == 5.0 ? paintFace(7, q, 1.5, vPat.w, pk * uPaintK.w) : paintFace(6, q, 1.3, vPat.w, pk * uPaintK.w * 0.6);
    } else if (pk > 0.0) {
      col *= pInk(friezeFace > 0.5 ? paintPanel(uv / max(sz, vec2(0.05)), pk * uPaintK.z) : paintWall(2, 2, q, 1.7, vPat.w, pk * uPaintK.z), uPaintInk);
      col *= 1.0 - 0.45 * wet * pk;
      emit += uFogBaseCol * wet * pk * (0.3 * smoothstep(0.5, 0.95, n.y) * (0.1 + 0.9 * pow(clamp(1.0 - V.y, 0.0, 1.0), 4.0)) + 0.4 * pRivulet(q, vert));
    }
  } else if (kind == 6.0) {
  ...
  // P5 paint: explicit surfaces (Look.surf) and bare stone (kind 9) — multiplied under the wash and the ink
  if (pk > 0.0 && kind != 1.0 && kind != 2.0 && kind != 3.0 && kind != 5.0) {
    float s = surf < 0.5 && kind == 9.0 ? 2.0 : surf;
    if (s == 2.0) {
      col *= pInk(paintWall(2, 2, q, 1.7, vPat.w, pk * uPaintK.z), uPaintInk);
      // rain-soaked stone: wet darkens it, its tops take a sheen of the silk sky
      col *= 1.0 - 0.45 * wet * pk;
      emit += uFogBaseCol * wet * pk * (0.3 * smoothstep(0.5, 0.95, n.y) * (0.1 + 0.9 * pow(clamp(1.0 - V.y, 0.0, 1.0), 4.0)) + 0.4 * pRivulet(q, vert));
    } else if (s == 3.0 || s == 6.0) col *= pInk(paintWall(4, 2, q, 6.0, vPat.w, pk * uPaintK.z * 0.9), uPaintInk);
...
export function jiehuaMaterial(shared: Shared, opt: { alphaCut?: boolean; viewmodel?: boolean; doubleSide?: boolean } = {}): ShaderMaterial {
  ...
  if (opt.viewmodel === true) {
    u['uPaintK'] = { value: new Vector4(0, 1, 1, 1) }; // P5 paint: the weapon is not painted stone
...
void main() {
  ...
  float px = fract(P.x);
  vec3 sky = mix(vec3(0.80, 0.86, 0.84), vec3(0.42, 0.62, 0.78), smoothstep(0.45, 1.0, P.y));
  vec3 c = sky;
  for (int i = 0; i < 5; i++) {
    float fi = float(i);
    ...
    if (P.y < h) {
      ...
      vec3 top = mix(vec3(0.06, 0.40, 0.33), vec3(0.08, 0.27, 0.56), step(0.5, fract(fi * 0.5 + uSeed * 0.13)));
      vec3 foot = vec3(0.70, 0.56, 0.36);
      vec3 cc = mix(top, foot, smoothstep(0.3, 1.0, t));
      cc = mix(cc, vec3(0.88, 0.9, 0.88), smoothstep(0.55, 1.0, t) * 0.8);   // the mist at its foot
      cc = mix(cc, sky, far * 0.55);
...
export function screenMaterial(shared: Shared, w: number, h: number, seed: number): ShaderMaterial {
  return new ShaderMaterial({
    uniforms: { ...shared.u, uSize: { value: new Vector2(w, h) }, uPitch: { value: 0.2 }, uSeed: { value: seed }, uFogScale: { value: 0.22 } },

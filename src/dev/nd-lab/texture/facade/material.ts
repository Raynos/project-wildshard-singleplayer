// P5 texture lab: a copy of the clean room's facade/material.ts; every change is marked `P5 paint`.
...
import { PAINT_GLSL } from '../paint'; // P5 paint
...
void main() {
  ...
  if (kind == 1.0) {
    ...
    // P5 paint: the glazed-tile texture's pitch (uTilePitch) for the rulings, no running-bond jog (the barrels align)
    vec2 tp = uTilePitch;
  ...
  // P5 paint: every concrete-ish surface of the dressing (the shell cells, far painted faces, parapet panels, plain
  // modules and slabs, AC boxes, slats, pipes) takes the painted concrete, world-anchored (walls: along x + z and up;
  // slabs: xz), so neither the cells' nor the pieces' seams show; glass, cloth, leaves, bars and signs keep their own
  if (kind != 2.0 && kind != 3.0 && kind != 4.0 && kind != 5.0 && kind != 9.0) {
    vec2 wq = abs(n.y) > 0.6 ? vWorld.xz : vec2(vWorld.x + vWorld.z, vWorld.y);
    col *= pInk(paintWall(4, 2, wq, 6.0, 0.0, uPaintK.x * uPaintK.z * 0.9), uPaintInk);
  }
...
void main() {
  ...
  if (!inGlass) {
    ...
    emitC = (litC - rc * lightC * 0.2) * lit * (1.0 - fres * 0.4) * 0.85 * (1.0 - cur * 0.5) + curLit * cur * lit * 0.45;
  ...
  vec3 avgLit = lightC * vec3(0.84, 0.72, 0.58) * 0.95;

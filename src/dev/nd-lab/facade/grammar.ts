...
const WARM = [0xffc57a, 0xffb866, 0xffd497, 0xf9a95c, 0xffc98a] as const;
...
export function dressWall(d: Dressing, p0: Vector3, n: Vector3, length: number, y0: number, y1: number, seed: number,
  opt: DressOptions = {}, depth = 12): Dressing {
  ...
  return dressTower({ x: c.x, z: c.z, w: length, d: depth, y0, h: y1 - y0, rot: Math.atan2(nn.x, nn.z), faces: 1 }, seed, opt, d);
}
...
function module(em: Emit, f: Face, cell: Cell, sc: number, y: number, bw: number, fh: number, wash: number, small: boolean, rng: Rng, street: number): void {
  ...
  for (const wi of wins) {
    ...
    if (caged) {
      ...
      if (small && rng.chance(0.3 * dens)) em.put('plant', f, wi.s + rng.range(-0.3, 0.3), sill - 0.06, 0.1, 0.8, 0.8, 0.8);
...
function gallery(em: Emit, f: Face, y: number, fh: number, bays: number, bw: number, wash: number, rng: Rng, small: boolean): void {
  ...
  for (let k = 0; k < bays; k++) {
    if (rng.chance(0.25 + 0.45 * em.timber)) em.put('lantern', f, (k + rng.range(0.3, 0.7)) * bw, y + fh - 0.3, D - 0.35, 1, 1, 1);

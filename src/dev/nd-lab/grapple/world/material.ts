...
import {
  Color, DataTexture, LinearMipmapLinearFilter, RedFormat, RepeatWrapping, ShaderMaterial, type Texture,
  type IUniform, UnsignedByteType, Vector2, Vector3, Vector4,
} from 'three';
import { Rng } from './rng';
...
function silkWeave(): DataTexture {
  ...
  for (let y = 0; y < N; y++) for (let x = 0; x < N; x++) {
    ...
    data[y * N + x] = Math.max(0, Math.min(255, Math.round(128 + thread * 60 + r.range(-12, 12))));
  }
...
/** uniforms shared by every facade program (one object each, so a write reaches every material) */
export function sharedUniforms(): Uniforms & {
  uCam: { value: Vector3 }; uLinePx: { value: number }; uTime: { value: number }; uFogDensity: { value: number };
  uFogBand: { value: Vector4 }; uFogCol: { value: Color }; uFogLow: { value: Color }; uLineFade: { value: Vector2 };
  uSilk: { value: Texture };
} {
  return {
    ...
    uTime: { value: 0 },
    uLinePx: { value: 1.3 },
    uLineFade: { value: new Vector2(32, 120) },
    uInk: { value: c(0x14161c) },
    uInkFar: { value: c(0x7a808c) },
    uLightDir: { value: new Vector3(0.35, 0.88, 0.3).normalize() },
    uShade: { value: c(0xa7b0c4) },
    uFogCol: { value: c(0x9eabc2) },
    uFogLow: { value: c(0x7d89a3) },
    uFogDensity: { value: 0.0085 },
    // the band below the street (the Well's depth): y, scale height, density
    uFogBand: { value: new Vector4(-8, 14, 0.05, 0) },
    uSky: { value: c(0xbcc4d2) },
...
// silk fog: a thin base haze plus an exponential band that thickens below uFogBand.x (the Well's depth, 留白 blank silk)
...
// an antialiased ruled line: d = distance to it (m), fw = metres per pixel, w = width (px); under 1 px it fades, never aliases
...
void main() {
  ...
  if (kind == 1.0) {
    ...
    col *= 1.0 - 0.1 * smoothstep(0.2, 0.6, streak) * smoothstep(0.1, 1.0, q.y / fh);
    ...
    float isLit = step(h12(cell + vPat.w), 0.3);
    ...
    emitC = vec3(1.0, 0.72, 0.4) * mix(0.3 * 0.3, win * isLit, det) * 0.9;
  ...
  vec4 fg = silkFog(vWorld);
  // alpha = the fog's transmittance: the composite fades the depth silhouette with it (no ink through blank silk)
  gl_FragColor = vec4(col * fg.a + fg.rgb + emitC * sqrt(fg.a), fg.a);
}
`;
...
void main() {
  ...
  if (!inGlass) {
    ...
    emitC = (litC - rc * lightC * 0.2) * lit * (1.0 - fres * 0.4) * 0.72 * (1.0 - cur * 0.5) + curLit * cur * lit * 0.35;

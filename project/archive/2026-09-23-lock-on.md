...
**State:** `archived` 2026-09-23 (finished 2026-09-23) — L1–L12 built and live (E50): the J LOCK disc (toggle), acquire / score / LOS / breaks / auto re-lock, the camera track with a feed-forward orbit and the Follow / Gentle / Off setting (Gentle default), orbit MOVE, target-relative DODGE, the lunge only onto the lock, the flick switch (touch, mouse, wheel), tap-an-enemy, the N HUD (▼ + brackets + tag, edge chevrons, SWITCH, ORBIT), Z / middle mouse, sounds + haptics, tests + scripts/lockon-drive.mjs (9 / 9). Leftovers: E74 (amber telegraph, perfect side-hop, locked jump attack, hold mode, Jake's iPhone feel check), E75 (the ranged lock).
...
## Built (2026-09-23, E50 — main session)

| Lever | State |
|---|---|
| L1 lock state / acquire | built — `lockOn` in AimTargets.ts, src/player/LockOnTarget.ts (hostile filter, 12 m, ±40° 3D, the score, LOS through src/physics/query.ts `lineOfSight`) |
| L2 break + auto-next | built — 18 m, 1.0 s out of sight, water / board / menu / ranged; a kill re-locks within 8 m ±90° after 0.35 s (Settings "Auto re-lock", on) |
| L3 camera track | built — engage 8 /s ≤ 240°/s until settled, hold ±2° at 6 /s ≤ 180°/s, × Follow 1 / Gentle 0.5 / Off 0 (Gentle default); FEED-FORWARD: the bearing change the player's own move causes is applied 1:1 (without it, Gentle lost the target mid-orbit); the ±10° / ±6° glance springs back |
| L4 orbit move | built — Player.step(): stick x round the target, y in / out, the radius held, never inside body + 0.9 m |
| L5 dodge / lunge | built — a side-hop of 3 m of arc, forward = a close-in dash, back = the backstep; Sword.findLunge only onto the lock |
| L6 flick switch | built — FlickTracker (≥ 28 px, ≥ 600 px/s, ≤ 200 ms), a ±60° sector, relative to the current target, 250 ms refractory; mouse flick > 40 px / 60 ms; the wheel |
| L7 LOCK disc (J) | built — off dim / available pulse / LOCKED filled; "No target" shakes it and re-levels the view |
| L8 locked HUD (N) | built — ▼ + brackets on the lock, a hollow ▽ on the candidate, the name + health tag pinned (Combat.ts), edge chevrons + distance (they flash on a miss), SWITCH ‹ ›, ORBIT arcs |
| L9 desktop | built — Z / middle mouse, the mouse flick, the wheel (verified headless: lock → wheel right → wheel left → unlock) |
| L10 sound + haptics | built — synth lock chime / switch ping / release tone / "nothing" tick (Audio.ts), 12 / 6 / 20 ms buzzes |
| L11 tap the enemy | built — a short tap on an enemy above the bar locks it |
| L12 tests | built — test/lockon.test.ts (11), scripts/lockon-drive.mjs (9 checks, headless + muted: available, lock + centred 0.53, flick switch, glance + spring back, orbit 153° with the radius ±0.2 m, centred 0.52, side-hop radius, auto re-lock, the 25 m break) |
| L13 hold mode / L14 perfect side-hop / L15 locked jump attack / the amber telegraph | not built — E74 |
| the ranged lock (§2.7 + Jake's Q7 note) | not built — E75 |

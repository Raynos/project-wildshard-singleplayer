...
| M3 | Creatures to their mockups: boar, bear, crab, monkey, drowned sailor — silhouettes, one material each (the draw-call fix), squash on hit | **done** — one material + one draw per creature, per-animal flash, monkey wind-up 0.41 s (`775dca1`), squash + crab claws (`ec96eff`); **the Drowned Captain generated** (v0.2): codex concept → Hunyuan3D-2 (full shape, 76 s + 13 s paint) → driftwood_post (9 k tris, 14 colours, AO) → `captain.glb` (306 KB), bound per vertex to the rig's nearest bone at load (`src/entities/species/captainMesh.ts`), the loft stand-in as fallback; phone at the spawn cove with him close: 127 calls / 0.96 M tris |

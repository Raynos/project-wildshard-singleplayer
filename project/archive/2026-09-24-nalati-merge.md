...
| # | row | state |
|---|---|---|
| A1 | The Nalati sound set: every Nalati sound (~40: wolves, horses, sheep, marmots, eagles, bow draw / release / impacts, sabre, javelin, hooves by ground, the steppe bed, wind, river, storm, thunder) generated with both models, the better take per sound, into main's one merged `sfx/best` set | ✅ 18b6c30 593589a (+ code 70875c0) — 51 families × both models, the better take per sound by CLAP: MOSS 24 · SA3 Medium 25 · synth 2 (spear thrust, javelin in flesh); +2.39 MB, tagged shard 'nalati' (decoded on the steppe only; Pine + Driftwood decode what they did); the bow's draw creak / full-draw click / let-down on H4's hooks; the Golden King's own call / hurt; scripts/music/gen/sfx_nalati.py, sfx-nalati.json |
| A2 | Music engine: a 4th music slot (Nalati), zone crossfades | ✅ 70875c0 b8d9358 — src/audio/SteppeScore.ts: Nalati's own score whatever the style (synth = synth), slots steppe-grass / sky / snow / night / storm / king picked by `music.setSteppe` (zone from SteppeAmbience.onZone, held 3 s; night; storm or Jel Ata; the King's barrow), decoded on demand (only the playing + wanted slot resident), ≥ 6 s crossfades on the bar; downloaded on the steppe only; herds no longer lift the score to alert |
| A3 | The score: a 6-take instrument test (dombra, kobyz, sybyzgy, a throat drone); then one Kazakh-folk score whatever the style setting, 3 zone variants (Nalati Grasslands / Sky Grassland / Snow Lotus Valley), calm / alert / combat / night / storm + stings; the throat drone only in the night + boss cues; one boss track each (the Golden King; Jel Ata = the storm cue); a listening page for the picks; the credit "Music: MiniMax-Music3" | ✅ 82769b8 0c55002 — the instrument test (sybyzgy + throat drone yes, dombra partly, kobyz no: MiniMax plays it plucked) and 24 score takes on art/music/round-4-nalati/index.html; the rank-1 take per slot ships (6.26 MB) **until the user picks** (name slot + seed); the Golden King's slot is its dirge-into-battle track, Jel Ata's = the storm cue |
| A4 | Zoned ambience for the three zones (like Driftwood's IslandAmbience) | ✅ 70875c0 — src/audio/SteppeAmbience.ts: Nalati Grasslands (grass wind, larks, the Kunes, the camp) · Sky Grassland (high wind, far herds, marmots) · Snow Lotus Valley (cold wind, meltwater, eagles); crickets at night, the storm's rain + gale over all; the synth bed stays the fallback (Settings ▸ Sound effects = Synth) |

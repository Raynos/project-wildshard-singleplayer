...
**State:** `archived` 2026-09-22 (finished 2026-09-22, **iPhone confirmation pending**) — every headless row passes: `bench.budget.json` **8 / 8** on Driftwood Isle, the default shard (the user: "default shard only"; tier=phone 390×844 4× CPU: title 0.46 s · 2.38 MB · warm transfer 0 · 17 requests · cold 4G 4.29 s · warm 4G 1.72 s · longest task 61 ms · SW), live `031e512`. Pine Hollow (experimental) best-effort: 11.7 MB, 17 requests. The one open item is the user's iPhone reading of a warm launch ≤ 3 s — ASKS E4 (closes L1); if it fails, this plan moves back to docs/plans/ as `in progress`. Leftovers: none beyond E4.

**2026-09-23 (E44):** P5's cold first-launch targets (≤ 12 MB, ≤ 20 requests, cold 4G ≤ 10 s) are lifted — the user picked completeness over cold bytes: every audio file (all styles, all SFX sets) and the title art now load at the bar, so Driftwood's cold launch is 16.1 MB / 161 requests / 19.5 s on 4G (project/archive/2026-09-23-preload-offline.md). `bench.budget.json` is re-set (each re-set row says why); L1 / P5 now target the cached launch.

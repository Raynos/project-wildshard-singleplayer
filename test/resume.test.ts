...
import { BRAND_KEY, LOOK_CLASS, RESUME_HTML, SHOT_KEY } from '../src/ui/Resume';
...
describe('resume screen', () => {
  ...
  it('the inline script reads the same reload flag and still key', () => {
    ...
    expect(html).toContain(`r.classList.add('${LOOK_CLASS}'`);
    expect(html).toContain(`localStorage.getItem('ws.settings.v1')`);
  });
